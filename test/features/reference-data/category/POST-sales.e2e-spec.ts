...
describe('test for POST sales', () => {
    ...
    it('should create new sale ', async () => {
        await categoryTestManagers.createCategory(token, {title: 'свеча 100мл'})
        await categoryTestManagers.createCategory(token, {title: 'свеча 200мл'})
        await categoryTestManagers.createCategory(token, {title: 'свеча 300мл'})
        const res: CategoryOutputModel = await categoryTestManagers.createCategory(token, {title: 'духи'})
        expect(res.title).toBe('духи')
